...
function mvobj_proto:walk(pos, speed, param, new_self)
	if new_self then
		s = new_self
	else
		s = self
	...
	if param then
		s:set_walk_parameter(param)
	end
	-- print('hello 2')
	s._target_pos_bak = s.target_pos
	s.target_pos = pos
	s.speed = speed
	if s.walk_param.find_path == true then
		s._path = s:get_path(pos)
	else
		s._path = { pos }
		s._path_used = false
	end
